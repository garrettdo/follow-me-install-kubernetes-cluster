...
``` bash
cat >> /etc/hosts <<EOF
192.168.128.231 k8s-01
192.168.128.232 k8s-02
192.168.128.233 k8s-03
EOF
```
...
``` bash
echo 'PATH=/data/k8s/bin:$PATH' >>/root/.bashrc
source /root/.bashrc
```
+ `/data/k8s/bin` 目录保存本文档下载安装的程序；
...
``` bash
mkdir -p /data/k8s/{bin,work} /etc/{kubernetes,etcd}/cert
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -P 61624 environment.sh root@${node_ip}:/data/k8s/bin/
