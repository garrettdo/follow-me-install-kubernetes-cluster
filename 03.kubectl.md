...
``` bash
cd /data/k8s/work
wget https://storage.googleapis.com/kubernetes-release/release/v1.18.0/kubernetes-client-linux-amd64.tar.gz # 自行解决翻墙下载问题
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -P 61624 kubernetes/client/bin/kubectl root@${node_ip}:/data/k8s/bin/
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -p 61624 root@${node_ip} "mkdir -p ~/.kube"
    scp -P 61624 kubectl.kubeconfig root@${node_ip}:~/.kube/config
  done
```